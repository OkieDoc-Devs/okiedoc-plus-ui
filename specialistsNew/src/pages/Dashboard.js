      ...
      <div className="main-content" id="app">
        ...
        <div className={`dashboard-content ${active === 'dashboard' ? 'active' : ''} container`} id="dashboard">
          <style>{`
            ...
            .rx-list, .lab-list { margin:0; padding-left:22px; }
        ...
        <div className="prescription-list" style={{ marginTop:'16px' }}>
          ...
          {mhRequests.length === 0 ? (
            ...
            <ol className="lab-list">
              {mhRequests.map((r)=> (
                <li key={r.id} className="prescription-item">
                  <div className="rx-item-title">{new Date(r.createdAt).toLocaleDateString()} — {r.status}</div>
                  {r.reason && <div className="rx-sig">Reason: {r.reason}</div>}
                  {(r.from || r.to) && <div className="rx-sig">Range: {r.from || '—'} to {r.to || '—'}</div>}
