  ...
  return {
    ...
    when:
      ticket.preferredDate && ticket.preferredTime
        ? `${ticket.preferredDate}T${ticket.preferredTime}:00`
        : ticket.createdAt
  ...
  const loadTicketsData = useCallback(async () => {
    ...
    try {
        ...
        const mappedActive = activeResponse.activeTickets.map((t) => ({
          ...
          when:
            t.preferredDate && t.preferredTime
              ? `${t.preferredDate}T${t.preferredTime}:00`
              : t.createdAt
      ...
      if (
        ...
        setApiError(null);
        saveTickets({ version: 1, tickets: allMappedTickets });
        return;
      } else {
        console.warn(
    ...
    const cached = loadTickets();
    const savedTickets = cached?.version === 1 ? cached.tickets : [];
    console.log(
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const dayTickets = tickets.filter((ticket) => {
        if (!ticket.preferredDate || !ticket.preferredTime) return false;
        if (ticket.status === 'Completed' || ticket.status === 'Cancelled')
          return false;

        const parsedDate = parseTicketPreferredDate(ticket.preferredDate);
          ...
          {selectedDate && (
            <div className='selected-date-panel'>
              ...
              <div className='day-schedules'>
                {(() => {
                  const dayTickets = tickets.filter((ticket) => {
                    if (!ticket.preferredDate || !ticket.preferredTime)
                      return false;
                    if (
                      ticket.status === 'Completed' ||
                    ...
                    const parsedDate = parseTicketPreferredDate(
                  ...
                  return dayTickets.map((ticket) => {
                    const ticketTime = ticket.preferredTime
                      ? (() => {
                          const [h, m] = ticket.preferredTime.split(':');
                ...
                return (
                  ...
                  >
                    ...
                    <div className='emr-patient-card__meta'>
                      ...
                      {t.preferredDate && t.preferredTime && (
                        <span className='emr-patient-card__meta-line emr-patient-card__meta-line--scheduled'>
                          <FaClock aria-hidden />
                          Scheduled:{' '}
                          ...
                          at{' '}
                          {(() => {
                            const [h, m] = t.preferredTime.split(':');
                            const hour = parseInt(h, 10);
                            const ampm = hour >= 12 ? 'PM' : 'AM';
                            const h12 = hour % 12 || 12;
                            return `${h12}:${m} ${ampm}`;
