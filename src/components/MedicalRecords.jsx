...
export default function PatientMedicalRecordsModal({
  ...
  const loadMedicalData = async (pId) => {
    ...
    try {
      if (sharedData) {
        const consultations = (sharedData.treatmentPlans || []).map((note) => {
          const tId =
            typeof note.ticket === "object" ? note.ticket.id : note.ticket;
          const ticketNum = tId ? `T-${String(tId).padStart(3, "0")}` : "T-000";

          const docName =
            typeof note.specialist === "object"
              ? `Dr. ${note.specialist.firstName || ""} ${note.specialist.lastName || ""}`.trim()
              : "Specialist";

          // Filter the other records to only show ones linked to THIS specific SOAP Note / Visit
          const relatedCerts = (sharedData.certificates || []).filter(
            (c) =>
              (typeof c.ticket === "object" ? c.ticket.id : c.ticket) === tId,
          );
          const relatedPrescriptions = (sharedData.prescriptions || []).filter(
            (p) =>
              (typeof p.ticket === "object" ? p.ticket.id : p.ticket) === tId,
          );
          const relatedLabs = (sharedData.labRequests || []).filter(
            (l) =>
              (typeof l.ticket === "object" ? l.ticket.id : l.ticket) === tId,
          );
          ...
          return {
            ...
            labRequests: relatedLabs,
            medicalCertificates: relatedCerts,
          };
        });

        setPastConsultations(consultations);
      } else {
        // --- 3. OLD LOGIC: Runs if no sharedData is provided (e.g. for the Patient/Nurse) ---
      ...
      // --- 4. SAFE PROFILE FETCH ---
      // We wrap this in its own try/catch so if the Specialist isn't allowed to hit this endpoint,
      // it doesn't crash the entire modal!
      try {
        ...
        if (profileResponse?.data) {
          ...
          setMedications(
            toPillList(
              profileData.currentMedications || profileData.medications,
            ),
          );
        }
      } catch (profileErr) {
        console.warn("Could not fetch patient profile details:", profileErr);
        if (patient) {
          setAllergies(toPillList(patient.allergies));
        }
      }
    } catch (loadError) {
